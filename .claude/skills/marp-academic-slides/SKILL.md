...
4. **再利用を先に探す**：新規作図・新規執筆の前に、似た図・概念スライドが過去の自分のデックや指定 pptx に無いか確認する。あれば描き直さず取り込む（図は `marp-reuse-figures` で `src/figNN-...` にPNGのままコピー、定番スライドは構成ごとコピーして字句を保つ）。取り込んだ媒体は必ず `src/` にコピーしてリンク切れを防ぐ。
   - **探す先・真似る先は `docs/revision-map_2026H1.md` の A群（添削濃厚）を優先**。A群はユーザーの添削が濃く反映された好みの正解集。書き始める前に似た用途のA群デックを2〜3本開いて型を掴む。C群（pptx変換のみ）はデザイン・文体の参照元にしない。
...
7. **デック完成時は全数検証3点セット**：①PNG全数目視 ②ファクトチェック（引用・数値・曜日を一次ソースで）③聴衆ペルソナレビュー（わかりにくい点・AIっぽい言い回しの除去）。詳細は references/patterns.md「完成前の全数検証」。
8. **使い方を提示**：プレビュー/PDF 出力コマンドを最後に1-2行で示す。
