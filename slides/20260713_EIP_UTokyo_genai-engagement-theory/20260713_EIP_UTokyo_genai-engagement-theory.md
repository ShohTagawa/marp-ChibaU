...
paginate: true
size: 16:9
header: '<div class="hdr-left">生成AIと教育</div><span class="hdr-event">東大EIP 理論編</span><img class="hdr-logo" src="./src/utokyo-eip-logo.png" alt="UTokyo EIP">'
footer: ''
style: |
  :root {
    --accent: #00736B;          /* テーマカラー（ティール／グリーン） */
    --accent-dark: #004D45;
    --accent-soft: #E3F1EF;
    --section-bg: #F1F8F7;
    --hdr-left-w: 19%;
  ...
  section .page-title { left: 15%; width: 36%; }
  ...
  /* ===== 表紙（cover-hero｜15min第4回スタイルに統一）=====
     ...
     theme/academic.css で定義。色は --accent（テーマ色）が自動で効くので上書き不要。 */
...
<div class="cv-kicker"><img src="./src/utokyo-eip-logo.png" alt="UTokyo EIP">東大EIP 理論編</div>
<div class="title-small">学生の学びを守り、教育をもっと面白くするために</div>
<div class="title-big">生成AIと教育</div>
</div>
...
<div class="title-event">東大EIP ｜ 理論編</div>
<div class="title-date">2026/7/13（月）</div>
...
- テーマは「生成AIと教育」。禁止か容認かという二択ではなく、先生方ご自身がどう"向き合う文脈"を作るか、を一緒に考える時間にしたいと思います。
-->
...
<div class="chapter-num">前半 ・ 関わり方</div>
...
- 逆に全面禁止も難しい。社会に出ればAIは使う前提です。Anthropicの分析では、すでに36%の職業タスクで何らかの形でAIが使われ、用途はスキル増強と自動化がおよそ6対4。事務も専門職も例外ではありません。
...
- 進め方：この後はまず①を実演で体感→②をAIの画像で確認→③は次章で個人情報として詳しく。原理的に残る理由は前の「だから“それっぽい”」と巻末の参考スライドで。
-->
...
- 評価＝複数の案を比べて良否を判断、創造＝新しいプランを提案。ここが人の領域です。気をつけたいのは、効率化それ自体が目的になること。空いた時間を右側＝人にしかできない所へ回せて、はじめて意味があります。
...
<div class="chapter-num">後半 ・ 授業設計</div>
...
- これは千葉大の記入規則で作った例ですが、ご自身の大学の記入規則に読み替えれば同じように使えます。学内で共有すればチームの資産に。
...
<!--
- まずGemini in Chrome。ブラウザに組み込まれ、今開いているページについてその場で質問できます。これは千葉大の画面ですが、皆さんのGoogleアカウントでも同じことができます。
...
- これは千葉大の事務マニュアルの例ですが、皆さんの大学でも同じです。新人の問い合わせが減り、更新はソースを差し替えるだけ。
...
- AIに解かせて満点が出るなら作り変えのサインです。さきほどのBloomでいう評価・創造＝人の領域へ寄せる。
-->
...
- 学生が授業の外側で一人で学ぶ時間＝自学・学び直しを支えます。これは千葉大の例ですが、皆さんの大学でも同じです。
-->
...
- 生成物をそのまま自分の成果として出すのは、この誠実さに反します。AIの役割は情報収集や整理まで。それをどう使うかは人間の技だ、というのは、多くの先生方が既に共有されている考え方でもあります。
